...
export const projects: Project[] = [
  // ── 01 ────────────────────────────────────────────────────────────────
  {
    ...
    headline: [
      ...
      { value: "29 Jul → 5 Sep", label: "engineering started, plan written, first delivery", note: "the branch opens 29 Jul, my operating document is 25 Aug, first delivery 5 Sep. The 12 days people quote is the last leg, not the whole build" },
    ...
    context: [
      "**This was the second model change in six months, and that matters.** The buyer app had run a local-seller marketplace until February 2026, when it was switched off and replaced with the intercity one described below. So by August the company was changing its model again, on a business that was six months old, with the team that had just done it once already.",
      ...
      "**And the geography said the same thing a different way.** Across 2026 the business took 3.89 orders per pin code across 5,130 pin codes. **Thirty-five per cent of those pin codes saw exactly one order, ever.** The fifty busiest were 8.8% of all volume and the single largest city was 2.7%. That is not a national business, it is a thin film spread over India, and it is why a warehouse serving two cities densely beats couriers serving everywhere sparsely. I should be precise about the arc though: the old marketplace was **deeply concentrated**, Uttar Pradesh alone 57 to 74% of it. The 2026 book was the deliberately national and thin one. So it is not that we sold nationally and then retreated. We went national on purpose, found it did not pay, and pulled back to two cities.",
    ],
    mechanisms: [
      ...
      { title: "One change I can trace from release to order book in hours", body: "Most of what shipped that week is unreadable in the data, because 81 orders over eight days will not carry a before-and-after. One is clean. The release that **removed the minimum order value and added a ₹75 delivery fee went out at 13:30 on 2 September, and the first order carrying that fee appears at 17:04 the same afternoon.** The ₹500 waiver is exact in the data too: the largest un-charged basket is ₹500.00 and the smallest charged one ₹503.20. Seven orders paid it, ₹525 in all, and baskets visibly bunch just above ₹500. Small, and it is the one place where a release, a threshold and a buyer response line up to the hour." },
      { title: "The van was where it actually failed", body: "Our own fleet attempted **19 delivery jobs across 16 of the 81 orders, in two runs. Six arrived and thirteen did not**, so under a third of attempts succeeded, and ₹6,284 of cash on delivery went uncollected. That is the operating failure, and it is not the one I had been naming. Supply worked: every consolidated purchase order to the brands landed. Demand worked well enough. Packing worked. Then 43 orders sat packed and ready and never left, and of the ones that did leave, most did not arrive. Two vans against a same-day promise was the constraint we had written down in the plan and then walked into anyway." },
      { title: "Things we shipped in the last week that nobody used", body: "Worth listing, because a shutdown makes this visible in a way a going concern does not. A quantity-discount ladder shipped on 4 September and **fired zero times across the 38 orders that followed it.** Of the catalogue we loaded, **1,214 of 1,368 products and 21 of 27 brands never sold once.** One order in 81 was ever rated. And the delivered status was never used at all, so orders went from dispatched straight to completed. We were still adding features to a funnel whose last step was broken." },
      ...
      { title: "A queue for broken promises", body: "We promised three to four days and ran a queue of every order past that. In the final weeks it held fifty-three orders, the oldest sixteen days old. Against a design that collected from brands in the morning and delivered the same day, the median order took **3.24 days** and the slowest took ten. I had been saying 22% of JIT orders were rejected and that this was the live problem. **That is wrong and I should correct it.** Sixteen of the eighteen rejections carry one identical timestamp, 9 September at 10:20, the morning after the last order was placed: it is a single sweep closing the book, not buyers being turned away. Inside the eight days the thing actually ran, **exactly one order in eighty-one was rejected.** The real operating failure was one step later, at the van." },
    ],
    metrics: [
      ...
      { value: "101.95 → 0 → 10", label: "orders a day before, during the gap, and after", note: "the marketplace stopped on 24 Aug, seven days before the new app shipped. Nothing was sold at all between 25 and 31 Aug" },
      { value: "+23.5% value", label: "in the launch region, matched 21-day windows", note: "89 orders to 81, but ₹78,763 to ₹97,253 and 67 buyers to 77, achieved in 8 active days against 21" },
      { value: "6 of 19", label: "own-fleet delivery attempts that arrived", note: "₹6,284 of cash on delivery uncollected. The van, not the warehouse, was the failure" },
      ...
      { value: "38 days", label: "first commit to first delivery", note: "29 Jul to 5 Sep. From my written operating plan on 25 Aug it was 12, and that is the number I had been quoting" },
      ...
      { value: "5.0%", label: "of carts in the region became orders", note: "80 of 1,593, but 738 of those carts were empty, so the real denominator is closer to 855" },
    ],
    honesty:
  ...
  // ── 02 ────────────────────────────────────────────────────────────────
  {
    ...
    headline: [
      ...
      { value: "143.9h → 10.9h", label: "median time for a seller to prepare and hand over", note: "13 times faster. Over the same months the courier leg never moved, 88 to 123 hours every month" },
      { value: "22.4%", label: "of orders died after the box was already packed", note: "the most expensive place to fail. A failing order also took 40% longer than a succeeding one" },
      { value: "748 km", label: "median distance a delivered order travelled, from 39 origins", note: "distance buys days, not failures: transit 2.0 to 6.2 days across bands while fulfilment stays flat at 50–54%" },
    ],
    ...
    mechanisms: [
      ...
      { title: "We fixed our half thirteen times over. The courier never moved.", body: "Splitting the order clock into its legs is the most useful thing I did with this data, and I did it too late. **The time a seller took to prepare and hand over an order fell from 143.9 hours at the median to 10.9, thirteen times faster.** Over the same months **the courier leg did not move at all: 88 to 123 hours, every single month.** So every hour we won, we won on our own side of the handover, and the half we were paying for was a constant. Two more facts from the same clock. **22.4% of orders died after the seller had already packed the box**, which is the most expensive place to fail. And **a failing order took 40% longer than a succeeding one**, 230 hours against 165, so the queue was telling us which orders were in trouble and nobody was reading it as a signal. Put together, this is the whole argument for stopping renting delivery." },
      { title: "Buyers told us, in their own words", body: "One reason code is worth more than the rest: **141 buyers cancelled their own order citing the distributor taking too long to confirm it.** Not price, not the product, not delivery. The acknowledgement step, which was running at about seventeen hours. When a shopkeeper writes your internal latency into his cancellation reason, the problem has stopped being operational and become the product." },
      { title: "The share of orders that actually arrived went up 2.9 times", body: "The cleanest summary of the whole programme: **the fulfilment rate went from 22.0% to 63.6% over five months while volume grew fourfold.** Both halves matter, because nearly tripling the odds that an order arrives while taking four times as many is the hard version of that problem. And the ceiling is the honest part: **36% still never arrived.** This was not a solved system when the company closed. It was a much less broken one." },
      { title: "The returns problem was also the retention problem", body: "This is the link I wish I had drawn a year earlier. **Half of all first-time buyers had their first order rejected.** And a buyer whose first order actually arrived came back at **41.6% against 33.6%** for one whose first order did not. So every parcel that failed was not only a freight cost, it was a customer who never formed the habit. The growth work and the fulfilment programme were pushing on the same number from opposite ends and reporting it separately." },
      { title: "Two facts that told us where to push", body: "Cash on delivery came back at **35.9%, fully prepaid at 6.2%**, 5.8 times worse, and part-paid orders sit exactly in between at 18.9%. That middle point is the useful bit: it is a dose-response, so the more of his own money a buyer has already put in, the less likely the parcel is to come back. That is what justified the prepaid-only rule after a single return, and it is also the argument for showing payment options before checkout rather than after. And the couriers were not equal: **21.9% at Ekart against 50.9% at ShipRocket** on the same flow, a 2.3-fold spread, with Delhivery carrying 61% of the volume at 29.4%. One more that was counter-intuitive enough to be worth checking twice: orders carrying a coupon came back **less** often, not more. And the obvious objection to the cash finding, that cash buyers simply order differently, does not survive: on a separate cut the average basket is ₹971 on cash against ₹974 prepaid, effectively identical, with returns still five times apart. Same size of basket, different payment, very different outcome. Prepaid share was then taken from 11.8% to 28.3%, and the return rate on cash orders themselves fell from 33.1% to 19.7%." },
    ...
    metrics: [
      ...
      { value: "22.0% → 63.6%", label: "orders that actually got fulfilled", note: "over five months, while volume grew fourfold. 36% still never arrived" },
      { value: "33.6% → 41.6%", label: "repeat rate when a buyer’s first order actually arrived", note: "and half of all first-time buyers had their first order rejected" },
      ...
      { value: "35.9% vs 6.2%", label: "return rate, cash on delivery against fully prepaid", note: "5.8 times. Part-paid orders sit exactly between at 18.9%, so the more a buyer has paid, the less comes back" },
  ...
  // ── 03 ────────────────────────────────────────────────────────────────
  {
    ...
    context: [
      "New buyers arrived through Meta ads without much trouble. The problem was everyone already in the contact base: roughly a million shopkeepers who had installed the app, ordered once or never, and gone quiet. Push notifications did not reach them, and I measured how badly, twice. My own report in June, taken from the push platform, traced 31 days to 25 June: **81.77 million sends and 130,000 opens**, with half never reaching the device and three in a thousand of the rest being tapped. The production database cannot confirm that number and I want to be straight about why: it keeps only 78 days of push history, beginning four days after that report ends, and it has **no opens column at all**, so opens were never available there. What the database does show, for the window it holds, is a different and worse failure: of **11.76 million notifications queued, 8.8% were confirmed delivered and 58% were never attempted**, and the largest campaign the company ever ran reached 9% of the people it was aimed at. Either way round, push was not reaching these shopkeepers. WhatsApp did. That is why the channel existed and why it was worth paying per message for reach push was giving away for free. From February 2026 I ran the channel end to end: audience, copy, templates, the vendor, the sending platform, the measurement, the city groups, and the sales desk that answered the replies.",
    ...
    mechanisms: [
      ...
      { title: "The channel was never the problem, and September proved it", body: "I only measured this after the company closed, using the marketing dashboard’s own channel rules against the order book. Of the shopkeepers who installed from a WhatsApp link, the share who went on to place an order was **2.8% in June, 4.0% in July, and 38.1% in September.** Eighty of two hundred and ten. Paid ads in the same September were 2.2%. What changed in between was not the messaging, it was what the message delivered people into: by September there was one seller, no per-brand minimums and a flat delivery fee. **WhatsApp had been putting shopkeepers in front of a store they could not check out of.** Two caveats that belong with that number: 210 installs is a small base, and the September relaunch went to Delhi and Gurgaon, an audience that was already warm and actually serviceable, so it is targeted re-engagement rather than a cold blast." },
      { title: "One buyer was three quarters of the revenue", body: "The same query settles an argument I had with myself in June. **75.5% of all the order value from June’s WhatsApp installs came from a single shopkeeper, and 62.0% of July’s from another.** For paid ads the largest buyer is 11 to 21%. So any revenue figure for this channel is one large shop wearing a channel’s clothes, which is exactly why I threw two orders worth ₹5.6 lakh out of my own daily report at the time. The instinct was right and the pattern held for four months. For WhatsApp, quote the conversion rate and not the rupees." },
      { title: "How big the attribution gap actually was", body: "I can now put a number on why I stopped reporting from the tracking link. The link credits WhatsApp with **1,321 installs in June 2026.** My buyer-by-buyer matching of the message exports against app activity found **19,534** in the same month. That is roughly **fifteen times**. Neither method is wrong: the link only sees people whose referrer survived the round trip through the Play Store, and the matching sees everyone who read a message and then opened the app. Fifteen times is the size of the decision I was making when I chose which one to publish." },
    ...
    metrics: [
      ...
      { value: "2.8% → 38.1%", label: "WhatsApp installs that placed an order, June against September", note: "80 of 210 in September, against 2.2% for paid ads the same month. The product changed, not the channel" },
      { value: "75.5%", label: "of June’s WhatsApp order value came from one shopkeeper", note: "62.0% in July. For paid ads the largest buyer is 11–21%. Quote the conversion rate, not the rupees" },
      { value: "1,321 vs 19,534", label: "June installs credited by the tracking link, and by matching buyers", note: "about fifteen times apart. That gap is why I stopped reporting from the link" },
      { value: "8.8% / 58%", label: "push notifications confirmed delivered, and never attempted", note: "of 11.76 million queued over 78 days. Queried. The reason to pay for WhatsApp" },
      { value: "81.77M → 130k", label: "push sends to push opens over 31 days, from my June report", note: "the push platform, not the database, which holds no opens column and no history that far back" },
  ...
  // ── 04 ────────────────────────────────────────────────────────────────
  {
    ...
    headline: [
      { value: "30% → 94%", label: "of verified phones that finished registering", note: "the signup form and an OTP tap removed, 26 May and 16 June 2026. Matched 21-day windows" },
      { value: "21,064 → 9", label: "people abandoning the form mid-way, per three weeks", note: "the screens they died on are named in the data: a location chooser and an onboarding scratch card" },
      { value: "2.52% → 1.43%", label: "installs that placed a first order, April to August", note: "the honest half. I fixed the step I was looking at and the one below it got worse" },
    ],
    ...
    mechanisms: [
      { title: "Delete the signup form and the OTP tap", body: "The name-and-shop-type screen appeared straight after the OTP and blocked the buyer before he had seen a product. It was removed: land on home, and collect the name in the checkout address form where it is actually needed. The OTP now sends itself as soon as a valid ten-digit number is typed, with no “Next” button, and logs the buyer in on verify. Both tickets were written on 13 June and shipped in version 8.179.0 on 15 June. **This is the one I can now put a number on, and it is the biggest number in the case.** A buyer row only exists once a phone is verified, so the share of those rows that reach a completed registration is exactly the rate this change was meant to move. On matched 21-day windows it went from **29.7% to 93.9%**, on 42,445 phones before and 35,456 after. Put the other way: **21,064 people abandoned the form mid-way in three weeks before the change, and 9 did after.** The data also shows it as two steps rather than one, 26 May and 16 June, each a single day wide: removing the intermediate tap took it 28.9% to 64.6%, and removing the form itself took it 57.3% to 94.2%. The screens people were dying on are named in the records, a location chooser and an onboarding scratch card. And there was a real cost: **capturing the shop name at signup fell from 58.8% to 9.3%**, which is the trade I made deliberately when I moved that question to the checkout address form." },
      { title: "Pick the language from the shop's location", body: "The app used to force Hindi on everyone through a full-screen popup that lost buyers at the first step. I specified deriving language from the state the shop is in, set by a database trigger across 38 states, with a manual choice never overwritten. The Hindi belt gets Hindi, the south, east and metros get a thin toggle. The bug that made it fail first time is the useful part: the app was writing its own default back over the database on every restart. Shipped in 8.195.0 on 26 June. What it fixed, measured by state: **before the change 38 to 46% of new buyers in West Bengal, Assam, Odisha and Karnataka were handed a Hindi app, and afterwards 0.1 to 0.9% were.** Across the whole base, language matching the shop\u2019s location went from 66-74% to 92-94%. One limit I should state rather than let a reader assume: the mapping covers 38 states but only resolves to Hindi or English, so a Bengali or Kannada speaker gets English, not their own language. That is better than wrong-language Hindi and it is not the same as vernacular." },
      { title: "A lower floor for the first order, with no app change", body: "First-time buyers should see a lower minimum than repeat buyers. I specified it as a computed field that shadows the minimum-order column, so the app asks the same question and gets a buyer-aware answer, and the change shipped without touching the client. Within days the company simplified it to a flat **₹300 first-order minimum** across brands, announced on 11 June. The scale of it is on record in the seller table, where the old values were kept: a per-seller ladder running from ₹1,000 to ₹20,000 was collapsed to a flat ₹500 across **161 of 172 sellers**, entirely as configuration, with no app release. Afterwards **85.4% of first orders were below the old ₹1,000 floor**, so the orders the ladder had been blocking were real. On first-order volume, use matched three-week windows: **509 a week to 723, up 42%.** An earlier read of 789 to 957 is two adjacent weeks of a noisy series and I would not quote it. I also had a median first-order figure the wrong way round: the median was already falling before the change and rose afterwards, so that claim is gone." },
      { title: "Work the cart the buyer already has", body: "From the 89% finding: a reminder between home page sections, an abandoned-cart section listing open carts and how far each is from the minimum (8.214.0, 18 July), groups for active-cart, add-address and incomplete-profile buyers, and a seven-day cart expiry on a consecutive-inactivity rule so stale carts stopped distorting every metric. Cart conversion was 0.6% of lifetime carts when I wrote the specification. Two things I have since checked and would rather say myself. **The cart reminder ran for one week**, 5,426 impressions to 1,847 buyers, **no recorded clicks at all**, and the fold has been inactive since. And the expiry sweep, which did work as plumbing, took 73,421 open drafts to 41,657 in fourteen days, a 43% cut at about 5,000 carts a day. But **an expired cart turned out to be a dead cart already**: of 7,789 buyers swept in a steady-state week, 4.8% built another cart within a fortnight and **0.65% placed an order**. That makes the sweep safe, and it also means the reminder was aimed at people who were never coming back." },
      { title: "Bring the tabs to the top of the home screen", body: "I specified a tab strip so the highest-converting content sits above the fold instead of below three scrolls. It shipped on 18 July in 8.214.0 as **₹1 Deals, Combos, Free Products, Top Brands, Refer & Earn**, with ₹1 Deals as the default tab, plus a hero carousel and a coupons section. Games were in my list and were dropped from the strip. **And then it never reached anybody.** It is still configured, still marked live, still sitting at the top priority, and in the telemetry it was **seen by 15 buyers and tapped by 6**. Shipped in the release notes, not shipped in reality. Top Brands has no measurable existence at all. I wrote the specification, watched it merge, and never checked that it rendered, which is the most basic thing a product manager owes a change. What did reach buyers on that screen were the individual shelves and banners, and those are where the numbers below come from." },
      { title: "Put the margin on the card, not at checkout", body: "The first item in my 1 July specification was to stop hiding discounts behind coupon codes: the real price has to be on the product card. Category tiles then carried the highest margin available in that category (18 July), brand cards carried their margins (29 July), and product cards carried an inline margin percentage. A discount a buyer only finds at checkout did not influence the add to cart. What I cannot claim is that a fatter margin sold more. Across the orderable range a higher margin correlates weakly with selling at all, and among the products that do sell it correlates **negatively** with units, which is the opposite of the intuition. The sharpest version of that: **the 15 to 20% margin band holds 2,471 products, a third of the whole range, and 25 of them sold in eleven weeks.** That is a pricing and assortment problem, not a discoverability one, and putting the margin on the card did not touch it." },
      { title: "Sort and filter a catalogue a shopkeeper can actually shop", body: "A category sidebar with an offers tab, sort options including the seller's margin slab, deep links straight to a category or product (8.121.0, 6 April), a “deal items” filter chip, and filter chips brought to the front of search (8.57.0, January). The category tree and richer facets were specified and left unfinished when the company closed. Two corrections I owe this section. **There were never upfront margin filters.** The filter sheet a buyer sees is category, brand, sort, shop type and maximum retail price, and the sorts are relevance, price up, name and price down. Margin is upfront as a **badge on the card and as margin-led shelves**, three of which are named for it, which is a real thing but not a filter, and I had been saying filter. **And asking for as many filters as possible is exactly what went wrong.** It produced 382 categories, 3,029 filters and 95,835 options built out of **922 distinct filter names for a few dozen actual attributes**, because brand, brandName and Brand Name were allowed to be three different filters. The result: **a quarter of orderable products cannot be reached by any filter chip in their own category, and 226 of 412 live categories contain nothing a buyer can order.** More facets is not more findable. I asked for breadth and should have asked for a controlled vocabulary first." },
      { title: "Support as its own tab, with a named person behind it", body: "Support replaced the profile icon in the bottom bar (8.177.0, 13 June), then got WhatsApp support and new help options (8.179.0, 15 June). I specified making the support screen configurable and assigning every buyer a relationship manager, round-robin across three people, with rules that show a banner naming the buyer's own manager. The assignment was backfilled to **1,465,882 of 1,516,628 buyers**, about 97%, and querying it now gives 1,472,439, so the number holds. **It is routing coverage and I should not call it a managed relationship.** Three employee records carry about 458,000 buyers each, which is 89% of everyone covered, and only around a hundred buyers a month ever tap through to the named person. What did measurably improve is the answer itself: **the median time to a reply a buyer could actually see went from 11.5 hours in January to 2.44 hours in July 2026.** That is the number I would defend about support, not the coverage figure." },
      { title: "Free gifts funded by the brand, with a progress bar", body: "I specified the gift engine: cross a brand's cart threshold and a free item attaches at ₹0, with a live progress pill, and detaches if the cart drops back. Pansari at ₹1,500 and ₹2,500, Chukde at 300 pieces for a bedsheet, Chaivik at four and eight units. It was reverted out of one release on 16 June and landed properly in 8.186.0 on 17 June. One piece of it I built and never shipped: a planner that works out how to split a cart so it clears more than one brand’s gift threshold at once, sitting in the offers sheet. It is on a branch dated 19 July 2026 and it was never merged, so it belongs here as something attempted rather than something delivered." },
      { title: "Show what is about to be ordered", body: "After the buyer taps pay, a sheet lists exactly which deliveries are about to be placed. Unchecking one saves that seller for later instead of deleting it. It fixed buyers being surprised by a second delivery they had not meant to order. I cannot prove that with data, and the reason is my own omission: **the review sheet ships with no tracking on it at all**, not one event, so there is no completion rate and no drop-off to read. The nearest proxy, cart to checkout, shows no step change around it. I am counting this as unmeasured rather than as a win." },
      { title: "Twenty-nine reasons to send a notification", body: "I wrote the catalogue of reasons to message a buyer, led by carts blocked below the minimum, stranded items and abandoned payments rather than lapsed-user broadcasts, with a hard cap of 8 to 10 notifications per buyer per day. I specified the pipeline behind it, which reads a nightly copy of the buyer base instead of the live database. It shipped as a campaign console with reusable groups, each carrying its own number: cart blocked by minimum, average gap ₹301; payment started and not finished, the highest intent in the system. The live catalogue holds **29 triggers**, not the 31 I had been saying, and I have corrected that. The uncomfortable part is downstream: of 11.76 million notifications queued across the 78 days the database retains, **only 8.8% are confirmed delivered, and 58% were never attempted at all.** The largest campaign the company ever ran reached **9% of the people it was aimed at**. The trigger with the most volume, a group-buying prompt, delivered 6.1% while every transactional trigger delivered 90 to 100%. Writing a good trigger catalogue does not help if the pipe underneath it is dropping more than half of what you hand it, and I did not check the pipe." },
      { title: "The app I was changing was serving a business five months old", body: "Worth being exact about the scale, because it changes what these changes were doing. The buyer app ran a **local-seller marketplace until February 2026, when that model was switched off** and replaced with an intercity one: brands shipping to shops by courier across India. Almost none of the old audience came across, **235 of 140,715 buyers**, so the new business started close to zero. It then went **74 orders in January to 1,321 in February to 7,198 in June**, with buyers going 71 to 3,728 and sellers 12 to 31, and peaked at about ₹49 lakh of orders placed in a month. So the signup and cart work on this page was done on a business that was five months old and growing fast, not on a mature one. Small absolute numbers, moving quickly, and I would rather say that than let a reader assume otherwise." },
      { title: "Reach was never the constraint", body: "The number that should have been on the wall the whole time. **1.16 million buyers were registered and 10,172 of them, 0.87%, ever placed an order.** In the launch region it is the same shape: 69,921 installs produced 527 orders, 0.75%. And inside the funnel the hole is specific rather than general: **90.2% of the buyers who added something to a cart never reached the payment step.** We spent on acquisition for a year against a conversion rate that never moved, and the thing between a cart and a payment was where the business actually leaked." },
      { title: "Buyers were not shopping where I had been building", body: "I spent months on search, categories and filters. Then I looked at where a product view actually comes from in a normal week: **the home-page shelves and the seller catalogue together are 85% of product views and 85% of add-to-carts, while search is 5.1% of views and 0.85% of adds.** The 340 buyers in that week who only searched and never browsed produced seven adds to cart and **no orders at all**; the buyers who searched and browsed converted best. So the surface I invested in was not the surface being used, and the shelves I had treated as merchandising were the actual storefront." },
      { title: "Navigation beat promotion, about two to one", body: "On the same screen in the same weeks, the plain category rail was reached and tapped by **56 to 64% of buyers, against 28 to 34% for the promotional strip of deals and offers** sitting beside it. Shopkeepers wanted to find a department, not be sold a campaign. The rest of the ranking is useful too: by distinct buyers clicking, games came first at 13,064, then a winners board at 4,659, ₹1 deals at 4,514, free gifts at 2,483 and referral last at 2,434. The best converting thing on the whole screen was a small ₹1 banner in the footer at **20 to 22% of the buyers who saw it**; the worst was a ₹100 referral banner, which fell from 1.19% to 0.17% across two months on 24,000 to 27,000 buyers reached. I had ranked the strip by what I wanted people to do. The data ranks it by what they wanted." },
      { title: "We broke the top of our own home screen and nobody noticed for weeks", body: "In the week of 20 July 2026 every fold at the top of the home screen falls 95 to 97% and never recovers, while brand shelves further down the same screen keep serving 17,700 buyers a month. I first recorded this as an unexplained anomaly. It has a cause: **the releases of 18 July.** Split by app build over the same three days, the icon-tap rate is 6.6 to 7.9% on the builds before and **0.1 to 0.3% on the builds after** — my own change, the one that put the tabs at the top, is what stopped the top of the screen working. The rest of that release was not wasted: product views per active buyer went 19.6% to 34.4% and cart adds 17.7% to 35.3%. But the share of buyers who ordered did not move, 1.94% against 1.89%. So I made the shelf busier, broke the entry points above it, and shipped both without checking either. A weekly look at my own surfaces would have caught it in days." },
      { title: "The draft-cart prize, measured without the trap", body: "**Between 31.5% and 37.8% of the carts placed in a week had been created more than a day earlier.** That is the honest size of the returning-cart opportunity, and it is worth stating how it has to be counted: the obvious query filters carts to those not yet archived, which makes older weeks look empty and recent ones look enormous. Done that way the same series reads zero for every week before August and then explodes, which is fiction. The figure above comes from creation and placement timestamps instead. Of buyers who clicked an entry point, 36 to 44% reached a cart the same day and 4.8 to 5.9% placed one, worth between ₹1.7 lakh and ₹7.6 lakh a week." },
      { title: "The metric watching the leak was wrong by 448 times", body: "The report that told us how many carts failed to meet the minimum returned **165,253 carts for the year. 368 of them were real.** The test was written as zero being less than the minimum, which is true of every empty draft cart ever created, so 99.8% of what it reported was nothing at all. Recomputed properly, the leak is large and was invisible: **25,633 non-empty carts stranded below the ₹500 minimum in June, 16,892 in July, 13,480 in August**, between 42% and 56% of every non-empty draft, each short by an average of ₹364 to ₹402. A buyer a few hundred rupees short of a floor is the easiest order in the business to win, and the number that should have been shouting about it was broken." },
      { title: "What all of this did not fix", body: "The changes above did what they were aimed at. The funnel\u2019s bottleneck used to be registration, which held at 27 to 35% of installs for eight straight months, and after June it is not registration any more. But **the share of installs that ever place a first order went from 2.52% in April to 1.43% in August.** I fixed the step I was looking at and the step below it got worse. The honest reading is that removing friction let more people through a door into a shop they still did not buy from, and the work I did not do was the one after the door." },
      { title: "I thought we staged rollouts. We did not.", body: "I have said that UI changes went to a few app-version groups before everyone. Checking the release tables, that is not what happened: **every over-the-air release in the year targeted every live version at once**, the field for restricting a release to specific versions has been unused since March 2025, and there were no mandatory releases or store pushes after April 2026. What is true is that a release reaches **52% of daily active buyers within a day and 69% by the third**, across 111 to 147 versions live at any time. So the blast radius was always everyone, and I was describing an intention rather than a mechanism. The measurement point stands: the daily report counted only carts created today by buyers active today, so a buyer returning each day to the same cart looked like decline, and I asked for lifetime carts of active buyers as a second row." },
    ],
    metrics: [
      { value: "322 → 2,099", label: "orders a week, 30 Mar to 8 Jun 2026", note: "the peak week. Then 800–960 in July and 640–830 in August" },
      { value: "74 → 7,198", label: "orders a month on the new intercity model, Jan to Jun 2026", note: "buyers 71 to 3,728, sellers 12 to 31. The previous local-seller model was switched off in February" },
      { value: "30% → 94%", label: "of verified phones that finished registering", note: "matched 21-day windows, 42,445 phones before and 35,456 after. Two steps, 26 May and 16 June 2026" },
      { value: "21,064 → 9", label: "people who abandoned the signup form mid-way, per three weeks", note: "49.6% of everyone who verified a phone, down to 0.03%" },
      { value: "509 → 723", label: "first-time buyers a week, either side of the lower minimum", note: "up 42% on matched three-week windows. An earlier read of 789 to 957 was two adjacent weeks of a noisy series" },
      { value: "38–46% → 0.1–0.9%", label: "new buyers handed a Hindi app in non-Hindi states", note: "West Bengal, Assam, Odisha and Karnataka. Language matching location went 66–74% to 92–94%" },
      { value: "25,633", label: "carts stranded below the minimum in one month", note: "June 2026, average shortfall ₹364–402. The report watching for this counted 165,253 and only 368 were real" },
      { value: "2.52% → 1.43%", label: "of installs that placed a first order, April to August", note: "the step I fixed moved. The step below it did not" },
      { value: "10,172 of 1.16M", label: "registered buyers who ever placed an order", note: "0.87%. Reach was never the constraint" },
      { value: "90.2%", label: "of buyers who added to a cart never reached payment", note: "the specific hole, rather than a general conversion problem" },
      { value: "8.8%", label: "of queued notifications confirmed delivered", note: "of 11.76 million over 78 days; 58% never attempted. The largest campaign reached 9% of its target" },
      { value: "0 → 4.17%", label: "orders carrying a free gift", note: "matched windows around the release. The 10.6% I had been quoting is August’s peak, not the change" },
      { value: "73.4k → 41.7k", label: "open draft carts after the expiry sweep", note: "26 July against 9 August, then steady at 42–47k" },
      { value: "1,472,439", label: "buyers routed to a named relationship manager", note: "97% of the base. But three records hold 89% of them, and about 100 buyers a month tap through. Routing, not a relationship" },
      { value: "252 → 571", label: "support tickets after the tab shipped, on a flat base", note: "in-app form 136 to 411. Median first reply got worse though, 1.75h to 3.36h. Reply time improved over the year, not because of this" },
      { value: "85% vs 5.1%", label: "product views from shelves and the seller catalogue, against search", note: "search-only buyers placed no orders at all that week" },
      { value: "15 seen, 6 tapped", label: "the home tab strip I specified and never checked rendered", note: "still configured, still marked live, at the top priority. It shipped in the notes and not in reality" },
      { value: "56–64% vs 28–34%", label: "buyers tapping the plain category rail against the promotional strip", note: "same screen, same weeks. They wanted to navigate, not be merchandised" },
      { value: "31.5–37.8%", label: "of carts placed in a week were created more than a day earlier", note: "the real size of the returning-cart prize, counted from timestamps rather than the archive flag" },
      { value: "922 / 25%", label: "distinct filter names, and orderable products no filter can reach", note: "382 categories, 3,029 filters, 95,835 options. 226 of 412 live categories hold nothing orderable" },
      { value: "2,471 / 25", label: "products in the 15–20% margin band, and how many sold", note: "a third of the range, 1% of it moving, over eleven weeks. A pricing problem, not a discovery one" },
  ...
  // ── 05 ────────────────────────────────────────────────────────────────
  {
    ...
    headline: [
      { value: "6.9% → 36.6%", label: "of orders carrying a funded coupon", note: "matched windows either side of the release. An earlier read of 2.9 to 29.5 used windows that did not match" },
      ...
      { value: "45.7%", label: "cut in item-coupon spend from a 37-minute config change", note: "₹29,233 to ₹15,875 a week, and orders did not fall. 28 Jul 2026, 12:53 to 13:30" },
    ],
    ...
    mechanisms: [
      ...
      { title: "Three: ₹1 products as a trial", body: "Ten products from ten brands, normally ₹50 to ₹150, repriced to ₹1. The point was not the discount, it was getting an unknown brand into a shop that would never buy a full case of it. It was pure configuration, no code, which is why it appears in no release note: the first ₹1 price in the database lands at about 17:30 on 15 May 2026, three weeks before any of us were saying it went live. Later it became the default tab on the home screen (18 July), then an add-on inside the cart (23 July), and under the warehouse model it was gated behind the minimum with one ₹1 item per order." },
      ...
      { title: "The best change I made took thirty-seven minutes and shipped no code", body: "On 28 July 2026, between 12:53 and 13:30, I changed the coupon configuration. **Item-coupon spend fell 45.7% the following week, from ₹29,233 to ₹15,875, or 3.68% of order value down to 1.93%.** Orders did not fall with it: placed orders moved 0.3% and the average order actually rose 3.6%. So roughly half the discount was buying nothing, and finding that took a configuration screen rather than a release. It is also the cleanest thing on this page, because I can date it to the half hour from the discount rows themselves, which is how I know it was not the app release two days later that people assumed was responsible." },
    ...
    metrics: [
      { value: "6.9% → 36.6%", label: "orders with a funded coupon", note: "re-measured on matched windows around the release timestamp. Conversion stayed flat, 2.31% against 2.21%" },
      ...
      { value: "0 → 35.3%", label: "orders with an item-level coupon", note: "matched windows around 17 Jul 2026. An earlier read of 49% was the first partial week" },
  ...
  // ── 06 ────────────────────────────────────────────────────────────────
  {
    ...
    mechanisms: [
      ...
      { title: "Referral, measured through the wallet", body: "There is no referral table in the production database, so I measured it through the three wallet payments it makes. Between 29 June and 31 August: **229 payments of ₹10 to referrers and 70 payments of ₹50** for a first delivered order. An earlier count of 400 welcome bonuses and 99 delivered-order payments was wrong on both legs; re-queried against the tagged wallet legs it is 229 and 70. The design is the part I would defend: the new shop gets its ₹100 immediately because someone signing up needs a reason to finish, and the referrer's real money waits for a delivered order because the referrer is the one who can game it. The result is not defensible: our own console reported 12,116 people received a code and 289 referred anybody." },
  ...
  // ── 09 ────────────────────────────────────────────────────────────────
  {
    ...
    mechanisms: [
      ...
      { title: "Drivers were never the constraint the funnel implied", body: "**11,310 driver app installs produced 1,264 completed verification forms and 49 riders actually working.** Acquisition cost nothing and the form was the wall, and it was one screen: **72.7% of the drop happens on a single step.** Naming that screen would have been a day of work. Over the same period trips per rider went from 8.6 to 14.1, so the fleet got more out of each rider while recruitment collapsed." },
  ...
  // ── 10 ───────────────────────────────────────────────────────────────
  {
    ...
    mechanisms: [
      ...
      { title: "Four in five searches return nothing, and the catalogue is why", body: "I have no before-and-after for the synonym layer, so I cannot claim it improved anything, and the reason is blunter than I first thought. **The event table holds nothing at all before 29 June 2026**, and every kind of event in it, carts, products, search, support, folds, starts within the same hour. It is one table cutover, not the start of search tracking and not a backend change. Nothing about buyer behaviour before that date is recoverable from events, so the synonym layer, which shipped in December 2025, can never be measured against what came before it. What the tracking shows is the standing state, and it is bad: **79.0% of real buyer searches returned zero products in the week of 6 July 2026, and 88.9% in the week of 3 August.** Worse, not better. The cause is not the matching. The master catalogue holds about 1.68 million rows, but **only 7,515 products were actually orderable**, and in a normal month buyers could really buy about a thousand of them. So a shopkeeper types a real product name, the synonym layer resolves it correctly, and there is nothing in stock to return. **No amount of synonym work fixes a search whose answer is an empty shelf.** The right next move was assortment, and I had been treating it as a language problem. **One caveat on that rate, which I only found by reading the release notes:** the search event double-fired, once while the buyer typed and again on submit, and that was not fixed until 8 September 2026, after these two weeks were measured. A half-typed query returns nothing, so the true zero-result rate is lower than 79%. It is still the right order of magnitude and still an assortment problem, but the number is an upper bound rather than a reading." },
    ...
    metrics: [
      ...
      { value: "79% → 89%", label: "of buyer searches that returned no products", note: "weeks of 6 Jul and 3 Aug 2026. An upper bound: the search event double-fired on typing as well as submit until 8 September" },
      { value: "7,515 of 1.68M", label: "catalogue rows a buyer could actually order", note: "about a thousand moved in a typical month. The empty results are an assortment problem, not a matching one" },
  ...
  // ── More work (tier 2) ────────────────────────────────────────────────
  {
    ...
    short: "Seventeen days from idea to first delivery, checked and exact. I built the ordering app myself and our callers built the cart on the phone against a half-tonne minimum. It worked as a mechanism and not yet as a business: 26 orders, and the low floor was the only order size that ever converted.",
    tier: 2,
    ...
    headline: [
      { value: "17 days", label: "idea to first delivery", note: "7 to 24 August 2026. Checked against the order book, and it is exactly 17" },
      { value: "19 of 26", label: "orders that sat exactly on the half-tonne floor", note: "and not one of the four orders above a tonne was ever delivered" },
      { value: "44%", label: "of ordered value died before it was delivered", note: "and 91% of that was distributors refusing the terms, not retailers changing their minds" },
    ],
    detail: [
      ...
      "**What \u201cgone quiet\u201d actually meant, now that I have queried it.** The brand had taken 428,677 orders since March 2023 and peaked at 22,898 a month in October 2024. By July 2026 it was down to 42 a month. So this was not a new channel, it was a collapsed one, and that is the honest framing of the opportunity.",
      "**The design decision that held.** The deliberately low half-tonne first order was the right call and the data is unusually clean about it: **19 of the 26 orders sat exactly on the floor**, and of the four orders above a tonne, **none was ever delivered.** Every larger basket died somewhere between the phone call and the truck. If I had set the floor where the distributors wanted it, the channel would have produced nothing at all.",
      "**Where it actually broke, and it was not the retailers.** Of ₹4,97,448 ordered, ₹1,19,046 was delivered. **44% of the value died before delivery and 91% of that loss was a distributor refusing the terms.** One refusal, worth ₹39,202, came down to a ₹10-per-bag gap, which is 0.9% on the price. The single biggest order, ₹97,596 for four tonnes and a fifth of everything the channel ever booked, was killed by a distributor demanding payment in advance. The retailers wanted the goods. The middle of the chain did not want the deal.",
      "**The unit economics, small and real.** Ten delivered orders moved 5.1 tonnes for ₹1,19,046 and a profit of ₹4,501, which is 3.78%. The reason it is thin is transport: **delivery ate 56% of the gross spread**, ₹5,800 against ₹10,301 of margin on the price ladder, because paying ₹1,131 a tonne out of a ₹1,900 spread leaves very little. That is the number I would put in front of anyone deciding whether to run this again, and it says the per-tonne payment was set too high, not that the channel cannot work.",
      "**The funnel, and the concentration risk behind the headline.** 226 distributors listed, 225 live accounts, 210 with a retailer mapped to them, 4,778 feed retailers inside 40 kilometres of one, and **22 who ordered, which is 0.46% of the reachable base.** One distributor produced 37.8% of all the revenue the channel earned, and the three largest by value ordered delivered nothing at all. A 226-distributor headline hides a business that was really one distributor deep.",
      "**Two things I cannot show you.** The books, not the database, hold what retailers actually paid for delivery, so the claim that the base price stayed lowest in market because delivery was billed separately is not something I can verify here. And every one of the 26 orders is recorded as placed by the buyer, because the calling team worked by signing in as the shop. So the order book says this was a fully self-serve channel when it was almost entirely agent-built, and the only trace of the real mechanism is that 18 of the 26 orders had an answered call the same day.",
    ],
  },
  {
    slug: "storefronts",
  ...
  },
  {
    ...
    detail: [
      ...
      "Three apps ship from one codebase: buyer, seller and employee. Over eleven months I merged **805 of 1,613 pull requests** and cut **242 release versions**, from 7.153 to 8.255. On the production branch the gate is absolute: **204 of the 204 release merges are mine**, every one. That is the last phase of a chain, and the chain is the job: naming the problem, deciding what to build, specifying it, sitting in design review, taking it through engineering, testing it, writing what changed, then cutting the release. None of those phases is a solo act and the code is the engineering team’s. What the release number measures is that nothing reached a shopkeeper without going through the whole chain. I owned the Play Store side too: listings, rollout, store-listing tests. I was the company's most frequent specification author, with **489 of 1,585 tickets, 192 of them full requirement documents**, and I wrote the release notes that told support and operations what had changed. Monthly bug inflow went from 114 to near zero, with the honest caveat that the bug list also shrank because the company shrank. I would not claim that one alone.",
    ],
  },
  {
    slug: "ai-pod",
