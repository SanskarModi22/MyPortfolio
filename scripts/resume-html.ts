...
const band = `<div class="band">${r.headline.map(h=>`<div class="stat"><b>${esc(h.value)}</b><span>${esc(h.label)}</span></div>`).join("")}</div>`;
const main = `
${band}
...
*{box-sizing:border-box}body{font-family:"Helvetica Neue",Helvetica,Arial,sans-serif;color:#1f1c18;font-size:7.78pt;line-height:1.21;margin:0}
...
.band{display:flex;flex-wrap:wrap;gap:0 8pt;margin:0 0 3pt;padding-bottom:2.5pt;border-bottom:1px solid #e1d9c9}
.band .stat{display:flex;flex-direction:column;line-height:1.05;padding:0}
.band .stat b{font-size:8.5pt}.band .stat span{font-size:6.5pt;color:#6b665e}
</style></head><body>
